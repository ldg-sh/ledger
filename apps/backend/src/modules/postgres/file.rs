...
use chrono::Utc;
use ledger_backend_entity::file::ActiveModel as FileActiveModel;
use ledger_backend_entity::file::{Entity as File, Model as FileModel};
...
impl PostgresService {
    pub async fn get_file(&self, file_id: &str, user_id: &str) -> AResult<Option<FileModel>> {
        let file = File::find()
            .filter(ledger_backend_entity::file::Column::Id.eq(file_id))
            .filter(ledger_backend_entity::file::Column::OwnerId.eq(user_id))
    ...
    pub async fn get_multiple_files(
        ...
        file_ids: Vec<&str>,
        user_id: &str,
    ) -> AResult<Vec<FileModel>> {
        let files = File::find()
            .filter(ledger_backend_entity::file::Column::Id.is_in(file_ids))
            .filter(ledger_backend_entity::file::Column::OwnerId.eq(user_id))
    ...
    pub async fn list_files(&self, path: &str, user_id: &str) -> AResult<Vec<FileModel>> {
        let files = File::find()
            .filter(ledger_backend_entity::file::Column::Path.eq(path))
            .filter(ledger_backend_entity::file::Column::OwnerId.eq(user_id))
            .all(&self.database_connection)
            .await?;
    ...
    pub async fn list_related_files(&self, path: &str, user_id: &str) -> AResult<Vec<FileModel>> {
        let files = if path.is_empty() {
            ...
            File::find()
                .filter(ledger_backend_entity::file::Column::Path.starts_with(path))
                .filter(ledger_backend_entity::file::Column::OwnerId.eq(user_id))
                .all(&self.database_connection)
                .await?
        };
    ...
    pub async fn copy_multiple(
        &self,
        source_files: Vec<FileModel>,
        new_path: &str,
    ) -> Result<(), AppError> {
