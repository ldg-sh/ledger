...
pub async fn list_files(
    ...
    let files = if deep {
        postgres
            .list_related_files(path_str.clone().as_str(), &authenticated_user.id)
            .await
    } else {
        postgres
            .list_files(path_str.clone().as_str(), &authenticated_user.id)
            .await
    };

    let folders = s3_service.list_directories(&full_path).await;
        ...
        if let Ok(folders) = folders {
            for folder in folders {
                ...
                let files_in_folder = if deep {
                    postgres
                        .list_related_files(&folder, &authenticated_user.id)
                        .await
                } else {
                    postgres.list_files(&folder, &authenticated_user.id).await
                };
