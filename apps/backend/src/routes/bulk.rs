...
pub async fn delete(
    ...
    let delete_res = s3_service
        .delete_multiple_files(
            files
                .iter()
                .map(|file| build_key_from_path(&authenticated_user, &file.path))
                .collect(),
        )
        .await;
    ...
    let db_delete_res = postgres_service.delete_multiple(files).await;
...
pub async fn r#move(
    ...
    for file in files.clone() {
        let source_key = build_key_from_path(&authenticated_user, &file.path);
        ...
        let res = s3_service.move_file(&source_key, &destination_key).await;
