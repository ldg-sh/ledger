...
pub async fn metadata(
    ...
    let key = build_key_from_path(&authenticated_user, format!("{}/{}", path, file_id).as_str());
...
pub async fn download(
    ...
    let key = match file {
        Ok(Some(f)) => build_key_from_path(&authenticated_user, format!("{}/{}", f.path, file_id).as_str()),
...
pub async fn download_full(
    ...
    let path = build_key_from_path(&authenticated_user, format!("{}/{}", path, file_id).as_str());
